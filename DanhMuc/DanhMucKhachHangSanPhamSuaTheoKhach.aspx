<%@ Page Title="" Language="C#" MasterPageFile="~/Layout/MasterPage.master" AutoEventWireup="true" CodeFile="DanhMucKhachHangSanPhamSuaTheoKhach.aspx.cs" Inherits="DanhMuc_DanhMucKhachHangSanPham_CapNhat" %>
    ...
    <link href="../dist/jquery-ui-1.11.3/jquery-ui.css" rel="stylesheet"/>
      <script>
          window.onload = function () {
              ...
              listsanpham('<%=Request.QueryString["IDChiTietGiaTheoKhach"].Trim()%>');
              //TenSanPhamTheoKhachAutocomplete();
          }


          function listsanpham(IDKhachHang) {
              var xmlhttp;
              ...
              xmlhttp.onreadystatechange = function () {
                  if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                      if (xmlhttp.responseText != "") {
                          $('#listsanpham').html(xmlhttp.responseText);
                      }
                      else {
          ...
          function TinhTienSo(IDLoaiHangHoa) {
              ...
              var t = $('#gia_' + IDLoaiHangHoa).val();
              if (t.trim().length <= 0) {
                  $('#gia_' + IDLoaiHangHoa).val("0");
              }
              if (isNaN(t)) {
                  $('#gia_' + IDLoaiHangHoa).val("0");
              }

              $('#gia_' + IDLoaiHangHoa).val($('#gia_' + IDLoaiHangHoa).val().replace(/\./g, '').replace(/\,/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, "."));

              var gia = $('#gia_' + IDLoaiHangHoa).val().replace(/\./g, '').replace(/\,/g, '');
          ...
          function CapNhatGiaoChoKhach(IDLoaiHangHoa) {
              ...
              if (check.disabled == true) {
                  document.getElementById("gia_" + IDLoaiHangHoa).disabled = false;
                  document.getElementById("hinhthoi_" + IDLoaiHangHoa).src = "../images/save.png";
              }
              else {
                  var Gia = $('#gia_' + IDLoaiHangHoa).val().replace(/\./g, '').replace(/\,/g, '');
                  var IDKhachHang = '<%=Request.QueryString["IDChiTietGiaTheoKhach"].Trim()%>';
                  var xmlhttp;
                  if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
                      xmlhttp = new XMLHttpRequest();
                  }
                  ...
                  xmlhttp.open("GET", "../Ajax6.aspx?Action=CapNhatGiaoChoKhach&IDLoaiHangHoa=" + IDLoaiHangHoa + "&Gia=" + Gia + "&IDKhachHang=" + IDKhachHang, true);
                  xmlhttp.send();
